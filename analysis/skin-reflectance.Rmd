...
```{r within-individual}
# find individuals with measurements at all 12 body locations
sub_ids = rle(issa$C)
for (iloc in bodys) {
  issa_body = issa %>% filter(G == iloc)
...
$$Y_i(t) = \sum_{j=1}^J X_{ij}(t)\beta_j(t) + \varepsilon_i(t),$$
where $t$ ranges from 400 nm to 700 nm, and $\beta_j(t)$ is the coefficient function for body location $j$, and $\varepsilon_i(t)$ is the residual error term.
...
kable(body_summary %>% select(body_location, n), col.names = c("Body location", "Number of individuals"), caption = "The number of individuals with measurements at different body locations")
```
...
for (i in 1:length(bodys_reg)) {
  iloc = bodys_reg[i]
  issa_body = issa_reg %>% filter(G == iloc)
  issa_within[,,i] = as.matrix(issa_body %>% select("400":"700"))
}
```
