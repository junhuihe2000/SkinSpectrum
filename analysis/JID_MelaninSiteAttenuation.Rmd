...
# workflowr sets knit_root_dir to "." (project root) in _workflowr.yml
# This ensures all file paths work consistently across different systems

# Install required packages (run once before first use)
...
Here we analyze data from the International Skin Spectra Archive (ISSA) published in Scientific Data (Lu et al., 2025 [https://doi.org/10.1038/s41597-025-04857-5](https://doi.org/10.1038/s41597-025-04857-5))--a collection of skin reflectances from over 2,000 individuals across multiple ethnicities and multiple body sites.

## Load data

First we load the data from the original Excel file and prepare it for analysis.

```{r load_issa}
# Load the ISSA skin spectrum reflectance data
issa_path <- file.path("data", "ISSA_17_Jan_2025_Yan_Lu.xlsx")
...
face_spectrum = face_spectrum %>% mutate(mi = 100 * `A680`,
                               ei = 100 * log10(`R680` / ((`R550` + `R560`)/2)))
# calculate far red melanin slope for face
fr_face_absorbance = face_absorbance %>% dplyr::select(`A650`:`A700`)
fr_face_coef = as.data.frame(t(apply(fr_face_absorbance, 1, function(x) {
  model = lm(x ~ fr_waves)
  return(coef(model))
})))
face_spectrum$s_mel = fr_face_coef$fr_waves
# calculate baseline-removed hemoglobin residual for face
hb_face_absorbance = face_absorbance %>% dplyr::select(`A630`:`A700`)
hb_face_pred = apply(hb_face_absorbance, 1, function(x) {
  model = lm(x ~ hb_waves)
  return(predict(model, newdata = data.frame(hb_waves = 570)))
})
face_spectrum = face_spectrum %>% mutate(Hb_resid = (`A560` + `A570` + `A580`)/3 - hb_face_pred)
# calculate wide-band Hb slope for face
face_spectrum = face_spectrum %>% mutate(E_wide = (`A600` - (`A520` + `A530`)/2)/(600 - 525))
# calculate E_wide tone-robust for face
model_face = lm(E_wide ~ s_mel, data = face_spectrum)
face_spectrum$E_wide_tr = -resid(model_face)
...
```{r within-individual}
# Get cheek and forehead data from the original spectrum, not face_spectrum
# face_spectrum is the average of cheek and forehead
cheek_spectrum = spectrum %>% filter(C %in% face_subject_ids, G == "Cheek")
cheek_spectrum = cheek_spectrum %>% group_by(C) %>%
  summarise(
    across(where(is.numeric), ~ mean(.x, na.rm = TRUE), .names = "{.col}"),
    .groups = "drop"
  )
forehead_spectrum = spectrum %>% filter(C %in% face_subject_ids, G == "Forehead")
...
delta_spectrum = cheek_spectrum %>% select(ei, E_wide, Hb_resid) - forehead_spectrum %>% select(ei, E_wide, Hb_resid)
...
```{r delta_E_wide-delta_hb correlation}
# Ordinary Least Squares
fit_ols_wide <- lm(delta_E_wide ~ delta_Hb_resid, data = delta_spectrum)
summary(fit_ols_wide)

ggplot(delta_spectrum, aes(x = delta_Hb_resid, y = delta_E_wide)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "steelblue", size = 1.2) +
  # geom_smooth(method = "rlm", se = FALSE, color = "tomato", linetype = 2, size = 1.2) +
  labs(
    x = "Delta Hb_resid",
    y = "Delta E_wide",
    title = "Relationship between Delta E_wide and Delta Hb_resid"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold"),
